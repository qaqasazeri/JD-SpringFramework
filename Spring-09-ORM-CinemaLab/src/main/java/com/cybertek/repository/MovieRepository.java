...
//Write a derived query to list all movies between a range of prices
    ...
    List<Movie> fecthAllBetweenPriceRange(BigDecimal price1, BigDecimal price2);

    //Write a JPQL query that returns all movie names
    @Query("select distinct m.name from Movie m")
    List<Movie> fetchAlMovieNames();

    // ------------------- Native QUERIES ------------------- //
    //Write a native query that returns a movie by name
    @Query(value = "Selec @ from movie where name=?1", nativeQuery = true)
    ...
    List<Movie> retrieveAllBetweenPriceRange(BigDecimal price1, BigDecimal price2);

    //Write a native query to return all movies where duration exists in the range of duration

    @Query(value = "select * from movie  where duration IN ?1", nativeQuery = true)
    List<Movie> retrieveAllByDurationIn(List<Integer> durations);
